...
import { PlayerEvents, SoundEvents } from './events';
 ...
 */
@Publisher()
export class AudioModule extends SRModule {
    /** 输入：时间流 */
    @Stream('input:ticker$')
    ...
    /** 输出：全局加载事件流 */
    @Bus('bus:loadEvent$$')
    public readonly loadEvent$!: Observable<SoundEvents>;

    /** 输出：全局播放事件流 */
    @Bus('bus:playEvent$$')
    public readonly playEvent$!: Observable<PlayerEvents>;
